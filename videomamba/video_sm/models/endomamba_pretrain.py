...
class EndoMamba(nn.Module):
    ...
    def __init__(
        ...
        # with_mask_learning
        self.mask_token = nn.Parameter(torch.zeros(1, 1, 1, embed_dim))  # shape [1, 1, 1, C]
    ...
    def forward_features_with_masktoken(self, x, mask: Optional[torch.Tensor] = None):
        B, C, T, H, W = x.shape
        x = self.patch_embed(x)  # [B, C, T, H', W']
        B, C, T, H, W = x.shape
        x = x.permute(0, 2, 3, 4, 1).reshape(B * T, H * W, C)  # [B*T, N, C]
        x = x + self.pos_embed  # Add spatial pos

        x = rearrange(x, '(b t) n c -> b t n c', b=B, t=T)

        if mask is not None:
            #print(f"mask.shape: {mask.shape}")
            #  用 mask_token 替换被掩码的块
            mask_reshaped = mask.view(B, T, -1).unsqueeze(-1)  # [B, T, N, 1]
            mask_token = self.mask_token.expand(B, T, x.shape[2], -1).to(x.dtype)
            x = torch.where(mask_reshaped, mask_token, x)  # masked positions replaced

        # Temporal position + Dropout
        x = rearrange(x, 'b t n c -> (b n) t c')
        temporal_pos = self.temporal_pos_embedding(x).unsqueeze(0).to(x.device)
        x = x + temporal_pos
        x = rearrange(x, '(b n) t c -> b t n c', b=B, t=T)

        x = self.pos_drop(x)

        # Forward through all layers（same as原版）
        residual = None
        hidden_states = x
        for idx, layer in enumerate(self.layers):
            if layer.bimamba:
                ...
                if residual is not None:
                    residual = rearrange(residual, 'b t n m -> (b t) n m')
            else:
                hidden_states = rearrange(hidden_states, 'b t n m -> b (t n) m')
                if residual is not None:
                    residual = rearrange(residual, 'b t n m -> b (t n) m')
        ...
        # 输出完整特征（含 masked 位置）
        frame_features = rearrange(hidden_states, 'b t n c -> b (t n) c', n=x.shape[2], t=T)
