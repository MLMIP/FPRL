...
MODEL_PATH = '/gaoxieping/dsh/FCRMamba/videomamba/video_sm/models/videomamba_ckpt'
_MODELS = {
...
class VisionMamba(nn.Module):
    ...
    def forward_features(self, x, inference_params=None):
        ...
        x = x + self.temporal_pos_embedding[:,:x.size(1)]
        ...
        # return hidden_states[:, 1:, :] # 1, 1568, 192

    def forward(self, x, inference_params=None):
        x = self.forward_features(x, inference_params)
        # print(f"x的维度为{x.shape}")
        # if self.only_cls_token:
        #     x = x[:, 0, :]
        #     print(f"only_class_token的维度为{x.shape}")
        if self.with_head:
            x = self.head(self.head_drop(x))
        return x[:,1:,:]#only_features
