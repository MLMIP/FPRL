...
def get_args():
    ...
    parser.add_argument('--embedding_weight', type=float, default=0.8, metavar='N',
                        help='weight for cosine similarity loss')
    parser.add_argument('--past_future_weight', type=float, default=20, metavar='N',
                        help='weight for past_future similarity loss')
    ...
    parser.add_argument("--pcf_total_frames", type=int, default=24, help="Total frame length used in PCF sampling")
    parser.add_argument('--sampling_rate', type=int, default=4)
    parser.add_argument('--output_dir', default='out/fprl_pretrain/',
...
def main(args):
    ...
    aux_decoder = AuxiliaryDecoder(attn_mul=4, num_blocks=1, embed_dim=model.encoder.embed_dim, num_heads=16).to(device)
    projector = VideoTemporalProjector(input_dim=model.encoder.embed_dim).to(device)
    mask_adapter = MaskGenerator(num_patches=392, embed_dim=model.encoder.embed_dim, mask_ratio=args.mask_ratio).to(device)
    ...
    if args.output_dir:
        ...
        args.output_dir += "fprl_{}_b{}_seqlen{}withTest_{}_{}/".format(
    ...
    # alignment projector: Ct -> Cs
    # 维度：tiny=192, small=384
    # align_proj = nn.Linear(192, 384, bias=False).to(device)
    ...
    dataset_train = build_pretraining_pcf_dataset(args)
    ...
    # model_without_ddp.align_proj = align_proj
    model_without_ddp.aux_decoder = aux_decoder
    model_without_ddp.projector = projector
    model_without_ddp.mask_adapter = mask_adapter
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            model,
            model_without_ddp.aux_decoder,   
            model_without_ddp.projector,    
            model_without_ddp.mask_adapter, 
            # model_without_ddp.align_proj,
            data_loader_train,
            ...
            past_future_weight=args.past_future_weight,  
        )
        if args.output_dir:
