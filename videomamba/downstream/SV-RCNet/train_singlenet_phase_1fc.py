...
from models import endomamba_recognition
...
class mamba_lstm(torch.nn.Module):
    def __init__(self):
        super(mamba_lstm, self).__init__()
        ...
        self.body = create_model('endomamba_small', pretrained=True)
        self.embed_dim = self.body.embed_dim
        
        self.projection = nn.Conv2d(2048, self.embed_dim, kernel_size=1)

        self.avgpool = nn.AdaptiveAvgPool2d(output_size=1)
        self.lstm = nn.LSTM(self.embed_dim, self.embed_dim, batch_first=True)
        self.fc = nn.Linear(self.embed_dim, 7)
        self.dropout = nn.Dropout(p=0.2)
    ...
    def forward(self, x):
        ...
        x = self.projection(x).view(-1, sequence_length, self.embed_dim, H, W)
        B, T = x.shape[0], x.shape[1]
        
        x = self.body(x)  # [B, T, N, M]->[B, T, M, H, W]
        x = rearrange(x, 'b t (h w) m -> b t m h w', b=B, m=self.embed_dim, h=H, w=W, t=T)
        
        x = self.avgpool(x).view(-1, sequence_length, self.embed_dim)
        self.lstm.flatten_parameters()
        x, _ = self.lstm(x)
        x = x.contiguous().view(-1, self.embed_dim)
        x = self.dropout(x)
        x = self.fc(x)
...
class vit_lstm(torch.nn.Module):
    def __init__(self):
        ...
        from models import build_vit_base_patch16_224
        body=build_vit_base_patch16_224()
        ...
        weight='../checkpoints/endofm_lv.pth'
...
def train_model(train_dataset, train_num_each, val_dataset, val_num_each):
    ...
    if args.test:
        ...
        #msg = model.load_state_dict(torch.load('cholec80.pth'), strict=True)
        print(msg)
