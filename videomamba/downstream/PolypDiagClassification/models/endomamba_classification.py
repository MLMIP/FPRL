...
MODEL_PATH = '/gaoxieping/dsh/FCRMamba/videomamba/out/fcrmamba_pretrain'
_MODELS = {
    "videomamba_s16_in1k": os.path.join(MODEL_PATH, "fcrmamba_small_b64_seqlen2withMl09__withteacher_MIX7/checkpoint-latest.pth"),
    "videomamba_t16_in1k": os.path.join(MODEL_PATH, "fcrmamba_tiny_b64_seqlen2withTest__withteacher_MIX7/checkpoint-latest.pth"),
    "videomamba_m16_in1k": os.path.join(MODEL_PATH, "fcrmamba_middle_b64_seqlen2withTest__withteacher_MIX7/checkpoint-latest.pth"),
}
...
class EndoMamba(nn.Module):
    ...
    def get_lr_params(self, base_lr, factor=1):
        params = [
            {"params": self.patch_embed.parameters(), "lr": base_lr * factor},
            # {"params": self.cls_token, "lr": base_lr * factor},
            {"params": self.pos_embed, "lr": base_lr * factor},
            # {"params": self.temporal_pos_embedding, "lr": base_lr * factor},
            # {"params": self.head.parameters(), "lr": base_lr},
        ]
        for layer in self.layers:
            params.append({"params": layer.parameters(), "lr": base_lr})
        if self.with_cls_token:
            params.append({"params": self.cls_token, "lr": base_lr * factor})
        if self.with_head:
            params.append({"params": self.head.parameters(), "lr": base_lr})
        return params
    ...
    def forward_features(self, x, inference_params: Optional[List[Optional[Tensor]]] = None):
        ...
        Returns:
            frame_features (Tensor): Extracted frame features.
        ...
        # x = x + self.pos_embed  # Add spatial positional embedding

        # # Add temporal positional encoding and reshape for processing
        # x = rearrange(x, '(b t) n m -> (b n) t m', b=B, t=T)
        # if inference_params is not None:
        #     temporal_pos = self.temporal_pos_embedding.encoding[inference_params.seqlen_offset: inference_params.seqlen_offset + T, :].unsqueeze(0).to(x.device)  # (1, T_n, C)
        #     x = x + temporal_pos  # (B*n, T, C) + (1, T, C) 
        # else:
        #     x = x + self.temporal_pos_embedding(x).unsqueeze(0).to(x.device)
