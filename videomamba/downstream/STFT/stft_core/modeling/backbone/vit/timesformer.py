...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x, get_all=False, get_attn=False):
        ...
        H = (x.shape[1]) // (T * W)
        xp = rearrange(x, 'b (h w t) m -> b t m h w', b=B, h=H, w=W, t=T)[0]
        # print(xp.shape)
...
def get_vit_base_patch16_224(cfg=None, no_head=True, **kwargs):
    patch_size = 16
    TRAIN_CROP_SIZE = 224
    NUM_FRAMES = 16
    vit = VisionTransformer(img_size=TRAIN_CROP_SIZE, num_classes=1000,
                            patch_size=patch_size, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4,
                            qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), drop_rate=0.,
                            attn_drop_rate=0., drop_path_rate=0.1, num_frames=NUM_FRAMES,
                            attention_type='divided_space_time', **kwargs)
    vit.attention_type = 'divided_space_time'
    ...
    #                     filter_fn=_conv_filter, img_size=TRAIN_CROP_SIZE, num_patches=vit.num_patches,
...
def get_aux_token_vit(cfg, no_head=False, **kwargs):
    patch_size = 16
    vit = AuxTokenVisionTransformer(img_size=cfg.DATA.TRAIN_CROP_SIZE, num_classes=1000,
                            patch_size=patch_size, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4,
                            qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), drop_rate=0.,
                            attn_drop_rate=0., drop_path_rate=0.1, num_frames=16,
                            attention_type='divided_space_time', **kwargs)
    vit.attention_type = 'divided_space_time'
    vit.default_cfg = default_cfgs['vit_base_patch16_224']
...
def build_vit_base_patch16_224():
    ...
    opt = parse_args()
    opt.cfg_file = "/research/d4/gds/zwang21/svt/segmentation/models/vit/configs/Kinetics/TimeSformer_divST_8x32_224.yaml"
