...
def build_resnet_fpn_p3p7_backbone(cfg):
    ...
    # weight = '../checkpoints/foundation_surgical_clips32k_endofm_plus/checkpoint.pth'
    # weight = '../checkpoints/endolv_20min/checkpoint.pth'
    # weight = '../checkpoints/endolv_1min_1gpu/checkpoint.pth'
    ...
    weight='/gaoxieping/dsh/EndoFM-LV/checkpoints/endofm_lv/checkpoint.pth'
...
@registry.BACKBONES.register("R-50-FPN-MAMBA")
def build_resnet_fpn_mamba_backbone(cfg):
    body = create_model(
        'endomamba_small',
        img_size=640,
        with_head=False, 
    ...
    # weight="/gaoxieping/dsh/EndoMamba/videomamba/out/endomamba_pretrain/endomamba_small_b48_seqlen16_withteacher_MIX7/checkpoint-latest.pth"
    weight="/gaoxieping/dsh/EndoMamba/videomamba/out/endomamba_pretrain/endomamba_small_b64_seqlen8_withteacher_MIX7/checkpoint-latest.pth"
    ckpt = torch.load(weight, map_location='cpu')
    if "teacher" in ckpt:
        ckpt = ckpt["teacher"]
    renamed_checkpoint = {x[len("backbone."):]: y for x, y in ckpt.items() if x.startswith("backbone.")}
...
#         num_frames=8,
#     )
#     weight="/gaoxieping/dsh/VideoMamba/videomamba/endo_sm/exp/k400/videomamba_middle_mask/videomamba_middle_mask_pt_f16_res224/checkpoint-latest.pth"
#     ckpt = torch.load(weight, map_location='cpu')
#     if "teacher" in ckpt:
#         ckpt = ckpt["teacher"]
#     renamed_checkpoint = {x[len("backbone."):]: y for x, y in ckpt.items() if x.startswith("backbone.")}
